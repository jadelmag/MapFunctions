...
class MapFunctions {
    ...
    createMap(target:string) {
        ...
        let base:string = 'osm';
        let lat:number = 39.4750;
        var lng:number = -0.3837;
        var zoom:number = 12;

        mapboxgl.accessToken = 'pk.eyJ1IjoiamFkZWxtYSIsImEiOiJjamdkbXZyYXAzNTMwMndsaXlsd3FkZW9kIn0.EtQvyeHCXtLkfxYtyWBDjQ';
        let myStyle:string = this.getBaseMap(base);
        // let myStyle:string = this.getBaseMapImported(base);
    ...
    // getBaseMapImported(name:string): any {
 
    //     switch(name) {
    //         case 'osm': return new OSMMap().getMap();
    //         default:
    //             return new OSMMap().getMap();
    //     }
    
    // }

    getBaseMap(name:string): any {
        let osm_map = {
            "version": 8,
            "name": "OSM Bright",
            "metadata": {
